...
def decode_nms(out, NMS_thre = 0.3,score_thre = 0.3,variance = [0.1,0.2],top_kN = 100):
    ...
    for bs in range(Batch):
        box_keep = boxes[bs,IoU_keep[bs,:]].view(-1,2)
        class_keep = top_k_class[bs,IoU_keep[bs,:]].float().view(-1,1)
        score_keep = top_k_score[bs,IoU_keep[bs,:]].float().view(-1,1)
        bs_pred = torch.cat((box_keep,class_keep,score_keep),dim = 1).detach().cpu().numpy()
        pred_out.append(bs_pred)
    return pred_out

def decode_result_all_class(out, NMS_thre = 0.3,score_thre = 0.3,variance = [0.1,0.2],top_kN = 100):
...
def match_result(pred,ground_truth ,NMS_thre = 0.3):
#         matching
...
def match_AP(pred,ground_truth ,IOU_thr =0.5):
#         matching
    nd = len(pred)
    ...
    #         ground boxes match with bounding boxes
            for i in range(len_bbox):
                ...
                for j in range(len_gt):
                    if iou(gt_loc[j,:], pred_loc, IOU_thr):
                        confu_matrix[pred_classes][int(gt_classes[j])] += 1
    ...
    return confu_matrix,tp,pred_score
