...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, planes, BatchNorm=True, stride=1, DownSample=None):
        ...
        self.conv3 = conv1x3(planes, planes)
        self.bn3 = nn.BatchNorm1d(planes)
        self.conv4 = conv1x3(planes, planes)
...
def backbone(cfg, in_planes, BatchNorm, stride=2):
    ...
    conv1 = nn.Conv1d(10, 64, kernel_size=7, stride=stride,
...
def multibox(backbones, cfg, num_classes):
    ...
    for k, v in enumerate(res_source):
        loc_layers += [nn.Conv1d(backbones[v].conv2.out_channels, cfg[k] * 2, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv1d(backbones[v].conv2.out_channels, cfg[k] * num_classes, kernel_size=3, padding=1)]
...
def build_SSD(phase, size, num_classes, cfg):
    ...
    # mbox = [5, 5, 5, 15, 15]
    # mbox = [6, 9, 12, 15, 15]
    base_, head_ = multibox(backbone(cfg['num_filters'], cfg['input_channels'], True, 2),
