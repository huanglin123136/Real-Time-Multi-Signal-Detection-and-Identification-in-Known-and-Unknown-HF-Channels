...
def test_signals(test_cfg):
    ...
    # evaluation
    test_net(test_cfg['saved_folder'], test_cfg['txt_name'], net, test_cfg['using_gpu'], testset,
...
def evaluation_loop( signal_num=(1163, 176, 92), th_iou=0.3):
    ...
    settings = {'feature_maps': [256, 128, 64, 32, 16], 'min_dim': 8192, 'steps': [32, 64, 128, 256, 512],
                ...
                'trained_model': './weights/ResNet_i1_8192_20191130_30000.pth', 'using_gpu': False,
                'test_data_root': './data/test_data_1115.npy', 'test_label_root': './data/test_label_1115.npy',
                'saved_folder': './test/', 'txt_name': 'test_0529.txt', 'labelmap': ['AM', 'SSB', 'PSK'],
                'visual_threshold': 0.1}
    res = main(settings['saved_folder'] + settings['txt_name'], th_iou, signal_num)
    # np.savetxt('./csv/am_0528_tl.csv', res[:, 0, :], delimiter=',')
    # np.savetxt('./csv/ssb_0528_tl.csv', res[:, 1, :], delimiter=',')
    # np.savetxt('./csv/psk_0528_tl.csv', res[:, 2, :], delimiter=',')
    mAP = 0
    for i in range(3):
...
if __name__ == '__main__':
    ...
    path = 'F:/CSPL/DataTransform/0526_huihe/testlabels_i10_0526'
    for i in range(4):
        get_signal_num(path + '_' + str(i) + '.npy', signal_num)
