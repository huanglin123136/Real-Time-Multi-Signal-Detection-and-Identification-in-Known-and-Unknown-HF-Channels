...
from data_augmentation_new import *
...
class SignalDetectionv2(data.Dataset):
    def __init__(self, data_root, label_root, data_aug=False, dataset_name='SignalDetedtion'):
        ...
        self.data,self.data_80, self.labels = self.load_json()
    ...
    def __getitem__(self, idx):
        ...
        if self.data_aug:
            ...
            if roll < 0.5:
                seq,seq_80, seq_label = sample_filplr(seq,seq_80, seq_label)
            roll = np.random.rand(1)
            if roll < 0.5:
                seq,seq_80, seq_label= sample_up_filplr(seq,seq_80, seq_label)
            seq,seq_80, seq_label = sample_jitter(seq,seq_80, seq_label)
    ...
    def load_json(self):
        ...
        if os.path.exists(self.data_root + '.npz'):
            ...
            data_,data_80 = concat_data(self.data_root)

        return data_,data_80, label_

    def pull_seq(self, idx):
        """
        return m x 8192 np.array
        """
        return self.data[idx],self.data_80[idx]
...
def detection_collate(batch):
    ...
    return torch.stack(imgs, 0), torch.stack(imgs_80, 0),targets


if __name__ == '__main__':
    data_set = SignalDetectionv2('D:/USTC/20200913/20210317/G35_recorder_npydata/traindata_i10_0317_1',
                                 'D:/USTC/20200913/20210317/G35_recorder_npydata//trainlabels_i10_0317_1', True)
    data_loader = data.DataLoader(data_set, 10,
                                  num_workers=1, shuffle=True,
                                  collate_fn=detection_collate, pin_memory=True)
    batch_iterator = iter(data_loader)
    sample1, sample80,target1= next(batch_iterator)
